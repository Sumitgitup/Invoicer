...
import { createClientService, getAllClientsService, getClientByIdService, deleteClientService, updateClientService } from '../services/client.service';
...
export const getClientByIdController = asyncHandler(async (req: Request, res: Response) => {
    if (!req.user) {
      res.status(401);
...
export const updateClientController = asyncHandler(async (req: Request, res: Response) => {
    ...
    const client = await updateClientService(req.params.id, req.user.id, req.body);
    res.status(200).json({success: true, data: client})
})
